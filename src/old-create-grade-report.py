...
TOTAL_POSSIBLE_SCORE = df['Max Score'].max()

DATE = current_dateTime

round_decimals = 4
def even_round_str(n):
    rounded = str(np.round(n, round_decimals))
    if len(rounded) < round_decimals + 2:
        return str(rounded) + '0' * (round_decimals + 2 - len(rounded))
    else:
        return str(rounded)

def output_letter_grade(df, gs_output):
    delta = np.round(TOTAL_POSSIBLE_SCORE-df['Overall Score'], 3)
    percent_of_curr_points = df['Overall Score'] / df['Max Score']
    if delta < 0:
    ...
    # output = np.append(output, [f"Your current percentage based on your scores so far is {np.round(df['Overall Score'], 3)} / {np.round(TOTAL_POSSIBLE_SCORE, 3)} * 100 = {np.round(100*df['Overall Score']/TOTAL_POSSIBLE_SCORE, 3)}%."])
    ...
    if not DROPS:
        ...
        output = np.append(output, ["This accounts for dropped assignments – the lowest lab score was dropped – and all possible extra credit (discussion attendance, midterm SETs bonus, final SETs bonus, and Lab 9)."])
...
def output_lab_grades(df, gs_output):
    lab_score = df.loc['Lab Total']
    output = [f'Labs are worth 25% of your grade. There are {MAX_LABS} labs total. Your lowest lab grade has been dropped.\n']
...
def output_midterm_exam_grade(df, gs_output):    
    ...
    output = f"""The Midterm Exam is worth 15% of your grade.
...
def output_final_exam_grade(df, gs_output):  
    output = ['The Final Exam is worth 25% of your grade.']
